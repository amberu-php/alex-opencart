...
var cart = {
	...
	'add': function(product_id, quantity) {
		$.ajax({
			url: 'index.php?route=amberu/product/ajaxGetProduct&product_id=' + product_id,
			//type: 'post',
			//data: 'product_id=' + product_id + '&quantity=' + (typeof(quantity) != 'undefined' ? quantity : 1),
			dataType: 'json',
			beforeSend : function () {
				var spinnerWrapperHtml = '<div id ="amberu-spinner-wrapper" style="position: relative; height: 135px;"></div>';
				amberuEasyModal.fill('#amberu-easy-modal-product', spinnerWrapperHtml);
				//disable buttons
				$('.amberu-easy-modal-btn-product-page').addClass('disabled');
				amberuEasyModal.open('#amberu-easy-modal-product');
				amberuSpinner.spinSpinner('#amberu-spinner-wrapper');
			},
			complete : function () {
				amberuSpinner.stopSpinner('#amberu-spinner-wrapper');
				//enable buttons
				$('.amberu-easy-modal-btn-product-page').removeClass('disabled');
			},
			success: function(json) {
				...
				amberuSpinner.stopSpinner('#amberu-spinner-wrapper');
				amberuEasyModal.fill('#amberu-easy-modal-product', json['product_html'], json['product_name'], modalValues);
				$('.amberu-easy-modal-btn-product-page').attr('href', json['product_href']);
				amberuEasyModal.open('#amberu-easy-modal-product', false);
				//amberuMyModal.open('#amberu-my-modal-pop-up-info-default', 2000);
			},
	...
	//don't need cart.checkPricelists() and cart.confirmedAdd() by now. 'cause cart.add() load product page with different funcs with same actions
	'checkPricelists': function(product_id, quantity) {
		$.ajax({
			...
			success: function(json) {
				if (!json['cart_pricelist_number']) {
					cart.confirmedAdd(product_id, quantity);
				}
				else {
	...
	//amberu
	'confirmedAdd' : function(product_id, quantity) {
		$.ajax({
