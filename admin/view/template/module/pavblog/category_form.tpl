...
<div class="tab-content">
	...
	<div class="tab-pane" id="tab-language-<?php echo $language['language_id']; ?>">
		<table class="table no-border">
			<tr>
				<td class="col-sm-2"><?php echo $objlang->get('Заголовок');?></td>
				<td class="col-sm-10">
...
<table class="table">
	...
	<tr>
		<td class="col-sm-2">Изображение</td>

		<td class="col-sm-10">
			<a href="" id="thumb-image" data-toggle="image" class="img-thumbnail"><img src="<?php echo empty($thumb)?$no_image:$thumb; ?>" alt="" title="" data-placeholder="<?php echo $no_image; ?>" /></a>
...
// Description
	<?php foreach ($languages as $language) { ?>
  <?php if ($ckeditor) { ?>
  ckeditorInit('input-description-language<?php echo $language['language_id']; ?>', '<?php echo $token; ?>');
  //ckeditorInit('pavblog_category_description<?php echo $language['language_id']; ?>', '<?php echo $token; ?>');
  <?php } else { ?>
  $('#input-description-language<?php echo $language['language_id']; ?>').summernote({ height: 160 });
	$('#pavblog_category_description<?php echo $language["language_id"]; ?>').summernote({ height: 150 });
	<?php } ?>
	<?php } ?>
</script>