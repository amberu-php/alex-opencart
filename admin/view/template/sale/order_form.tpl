        ...
        <form class="form-horizontal">
          ...
          <div class="tab-content">
            <div class="tab-pane active" id="tab-customer">
			  ...
			  <!--//AMBERU-->
			  <div class="form-group">
                <label class="col-sm-2 control-label" for="amberu-order-form-pricelist-number"><?php echo $amberu_text_pricelist_number; ?></label>
                <div class="col-sm-10">
					<select class="form-control" id="amberu-order-form-pricelist-number" name="amberu_pricelist_number">
							<option value="0" 
						    <?php echo ($amberu_pricelist_number <= 0) ? 'selected="selected"' : ''; ?>
							>
							  <?php echo 'Отсутствует = Розница'; ?>
						    </option>
					<?php foreach ($amberu_pricelists as $amberu_key => $amberu_pricelist) { ?>
						    <option value="<?php echo $amberu_key; ?>" 
						    <?php echo ($amberu_pricelist_number == $amberu_key) ? 'selected="selected"' : ''; ?>
							>
							  <?php echo $amberu_pricelist['order_limit_text']; ?>
						    </option>	  
					<?php } ?>
					</select>
                </div>
			...
			<style type="text/css">
				...
				#cart td[amberu-td="product-quantity"] {
					position: relative;
				}
				#cart td[amberu-td="product-quantity"] div[class="text-danger"] {
					padding-bottom: 3px;
					font-size: 13px;
					font-weight: 600;
					text-align: center;
				}
				#cart i[amberu-fa="status"] {
					position: absolute;
					color: #0E9C52;
					font-size: 30px;
					left: 10px;
				}
				body i[amberu-fa="status"] {
					display: block;
					position: fixed;
					color: #0E9C52;
					font-size: 150px;
...
$('#button-refresh').on('click', function(e, amberuEvent) {
	//AMBERU
	amberuEvent = typeof amberuEvent !== 'undefined' ? amberuEvent : {'eventType' : ''};
	
	var amberuHelper = {
		'statusTransitionDelay' : 500,
		'statusShowDelay' : 500,
		'showSuccessStatus' : function(target) {
			target = $(target);
			if (!target.children('i[amberu-fa="status"]').length) {
				var statusNode = $('<i class="fa fa-check" amberu-fa="status"></i>').appendTo(target);
				statusNode.fadeOut(0).fadeIn( amberuHelper.statusTransitionDelay );
			}
			else {
				target.children('i[amberu-fa="status"]').fadeIn( amberuHelper.statusTransitionDelay );
			}
			window.setTimeout(function(){
				target.children('i[amberu-fa="status"]').fadeOut(amberuHelper.statusTransitionDelay);
			}, amberuHelper.statusTransitionDelay + amberuHelper.statusShowDelay);
		}
	}
	...
	$.ajax({
		...
		dataType: 'json',
		complete : function() {
			if (amberuEvent.eventType === 'productEdit') {
				//show success fa
				amberuHelper.showSuccessStatus($(document.body));
				$('#button-product-add').removeClass('disabled');
		...
		success: function(json) {
			...
			if (json['products']) {
				for (i = 0; i < json['products'].length; i++) {
					...
					//amberu
					html += '  <td class="text-right" amberu-td="product-quantity">' 
						+ (!product['stock'] ? '<div class="text-danger">Остаток: ' + product['amberu_store_quantity'] + '</div>' : '') 
						+ '<input class="form-control" amberu-input="product-quantity" value="' + product['quantity'] + '" />' 
						+ '<button amberu-btn="product-edit" type="button" value="' + product['key'] + '" data-toggle="tooltip" title="" data-loading-text="<?php echo $text_loading; ?>" class="btn btn-primary" data-original-title="Применить">' 
						+ '<i class="fa fa-check"></i>' 
						+ '</button>' 
						+ '<input type="hidden" name="product[' + i + '][quantity]" value="' + product['quantity'] + '" /></td>';
					//end
		...
		success: function(json) {
			...
			if (json['error']) {
				...
				$.ajax({
					...
					type: 'post',
					data: $('#amberu-order-form-pricelist-number, #cart input[name^=\'product\'][type=\'text\'], #cart input[name^=\'product\'][type=\'hidden\'], #cart input[name^=\'product\'][type=\'radio\']:checked, #cart input[name^=\'product\'][type=\'checkbox\']:checked, #cart select[name^=\'product\'], #cart textarea[name^=\'product\']'),
					dataType: 'json',
					beforeSend: function() {
...
//AMBERU
$('#tab-cart').on('click', 'button[amberu-btn="product-edit"]', function(e) {
	//var key = 'key=' + encodeURIComponent(this.value);
	//var postArray = $.param($('#tab-product input[name=\'product_id\'], #tab-product input[name=\'quantity\'], #tab-product input[name^=\'option\'][type=\'text\'], #tab-product input[name^=\'option\'][type=\'hidden\'], #tab-product input[name^=\'option\'][type=\'radio\']:checked, #tab-product input[name^=\'option\'][type=\'checkbox\']:checked, #tab-product select[name^=\'option\'], #tab-product textarea[name^=\'option\']'));
	var quantity = $(this).parent().children('input[amberu-input="product-quantity"]').val();
	var eventNode = this;
	$.ajax({
		...
		type: 'post',
		data: 'key=' + encodeURIComponent(this.value) + '&quantity=' + quantity,
		dataType: 'json',
		beforeSend: function() {
			$('#button-product-add').addClass('disabled');
			$('#button-cart').addClass('disabled');
			$(eventNode).button('loading');
			$('#cart .btn-danger, #cart button[amberu-btn="product-edit"]').addClass('disabled');
		},
		complete: function() {
			//$('#button-product-add').removeClass('disabled');
			//$('#button-cart').removeClass('disabled');
			//$(eventNode).button('reset');
		},
		success: function(json) {
			$('.alert, .text-danger').remove();
			$('.form-group').removeClass('has-error');
			...
			if (json['error']) {
				...
				// Refresh products, vouchers and totals
				$('#button-refresh').trigger('click', {'eventType' : 'productEdit'});				
				
				//show success fa
				//$(eventNode).parent().append('<i class="fa fa-check-circle" amberu-fa="status"></i>').hide().fadeIn( 100 );
			}
		},
		error: function(xhr, ajaxOptions, thrownError) {
			alert(thrownError + "\r\n" + xhr.statusText + "\r\n" + xhr.responseText);
