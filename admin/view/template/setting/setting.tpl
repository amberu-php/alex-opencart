      ...
      <div class="panel-body">
        <form action="<?php echo $action; ?>" method="post" enctype="multipart/form-data" id="form-setting" class="form-horizontal">
          <ul class="nav nav-tabs">
			...
			<li><a href="#tab-amberu-multiprice" data-toggle="tab"><?php echo $tab_amberu_multiprice; ?></a></li>
          ...
          <div class="tab-content">
			...
			<!--AMBERU-->
			<style type="text/css">
				#tab-amberu-multiprice .col-sm-4 {
					width: 33.3333%;
				}
				#amberu-multiprice-label {
					font-size: 14px;
				}
				h1, h2, h3, h4, h5, h6 {
					color: #666;
			 ...
			 <div class="tab-pane" id="tab-amberu-multiprice">
              <h2>AMBERU MULTIPRICE</h2>
			  <br/>
			  <div id="amberu-pricelists-container" class="form-group">
			  <?php 
				if (($config_amberu_pricelists == "") || ($config_amberu_pricelists == null)) {
					$config_amberu_pricelists[1]['order_limit'] = 0.00;
					$config_amberu_pricelists[1]['customer_group_id'] = -1;
				}
					$amberu_pricelist_count = 0;
					foreach($config_amberu_pricelists as $amberu_pricelist) {
						$amberu_pricelist_count++;
				?>
						<div id="amberu-pricelist-<?php echo $amberu_pricelist_count; ?>-container" class="form-group amberu-pricelist-form-group">
							<h5>Прайслист № <?php echo $amberu_pricelist_count; ?></h5>
							<label class="col-sm-2 control-label" for="input-amberu-pricelist-<?php echo $amberu_pricelist_count; ?>-order-limit">
								<span data-toggle="tooltip" title="<?php echo "Лимит прайслиста №", $amberu_pricelist_count; ?>"><?php echo "Лимит ", $amberu_pricelist_count; ?>
								</span>
							</label>
							<div class="col-sm-4">
								<input type="text" name="config_amberu_pricelists[<?php echo $amberu_pricelist_count; ?>][order_limit]" value="<?php echo $amberu_pricelist['order_limit']; ?>" 
									placeholder="<?php echo "Сумма"; ?>" id="input-amberu-pricelist-<?php echo $amberu_pricelist_count; ?>-order-limit" class="form-control" 
									onkeyup="amberuTrimInputValue(this)"
								/>
							</div>
							<label class="col-sm-2 control-label" for="select-amberu-pricelist-<?php echo $amberu_pricelist_count; ?>-customer-group">
								<span data-toggle="tooltip" title="<?php echo "Доступ к прайслисту №", $amberu_pricelist_count; ?>"><?php echo "Доступ"; ?>
								</span>
							</label>
							<select name="config_amberu_pricelists[<?php echo $amberu_pricelist_count; ?>][customer_group_id]" class="form-control col-sm-4" id = "select-amberu-pricelist-<?php echo $amberu_pricelist_count; ?>-customer-group">
							  <?php if ($amberu_pricelist['customer_group_id'] == -1) { ?>
										<option value="-1" selected="selected"><?php echo "Для всех"; ?></option>

							  <?php	} 
									else { ?>
										<option value="-1"><?php echo "Для всех"; ?></option>
							  <?php	} ?>
							  <?php	foreach ($customer_groups as $customer_group) { ?>
							  <?php 	if ($customer_group['customer_group_id'] == $amberu_pricelist['customer_group_id']) { ?>
											<option value="<?php echo $customer_group['customer_group_id']; ?>" selected="selected"><?php echo $customer_group['name']; ?></option>
							  <?php 	}	
										else { ?>
											<option value="<?php echo $customer_group['customer_group_id']; ?>"><?php echo $customer_group['name']; ?></option>
				...
				<div id="amberu-btn-group">
					<button id="amberu-add-item" type="button" onclick = "amberuAddItem('amberu-pricelists-container')" class="btn btn-primary">Добавить</button>
					<button id="amberu-remove-item" type="button" onclick = "amberuRemoveItem('amberu-pricelists-container')" class="btn btn-danger">Удалить</button>
				</div>
            </div>
			<script type="text/javascript">
				function amberuCheckboxChange(el) {
					if (el.checked) {
						document.getElementById('amberu-pricelists-container').style.cssText = 'display: block;';
						document.getElementById('amberu-btn-group').style.cssText = 'display: block;';
					}
					else {
						document.getElementById('amberu-pricelists-container').style.cssText = 'display: none;';
						document.getElementById('amberu-btn-group').style.cssText = 'display: none;';
					}
				}
				function setItemCount(operation, itemCountId) {
				...
				function getItemNumber(item, patternItemNumber){
					//var patternItemNumber = /pricelist-(\d*)/ig;
					var matches = patternItemNumber.exec(item.id);
					return itemNumber = matches[1];
				}
				function getLastItem(children, patternItemId) {
					var result = null;
					for (var i = children.length-1; i >= 0; i--) {//children.length-1 coz index starts from 0
						var matches = patternItemId.exec(children[i].id);
						if (matches != null) {
							result = children[i];
							break;
						}
				...
				function amberuRemoveItem(mainParentId) {
					//Remove last item
					//Get parent item that includes inputs
					var itemParent = document.getElementById(mainParentId);
					//Get lastItemContainer
					var lastItemContainer = getLastItem(itemParent.children, /amberu-pricelist-\d+-container/ig);
					if (lastItemContainer != null)
						var itemNumber = getItemNumber(lastItemContainer, /pricelist-(\d*)/ig);
					if (itemNumber > 1) {
						itemParent.removeChild(lastItemContainer);
						//setItemCount("decrement");
					}
					else alert("Amberu Multiprice: Спокойно. Полей для удаления больше нет. Минималное количество полей(прайс-листов): 1.");
				}
				function amberuAddItem(mainParentId) {
					//Get parent item that includes inputs
					var itemParent = document.getElementById(mainParentId);
					//Get lastItemNumber and newItemNumber
					var lastItemContainer = getLastItem(itemParent.children, /amberu-pricelist-\d+-container/ig);
					if (lastItemContainer == null) {
						var newItemNumber = 1;
					}
					else {
						var lastItemNumber = getItemNumber(lastItemContainer, /pricelist-(\d*)/ig);
						var newItemNumber = parseInt(lastItemNumber) + 1;
					}
					//alert(newItemNumber);
					var newItem = document.createElement('div'); 
					newItem.setAttribute('id','amberu-pricelist-' + newItemNumber + '-container');
					newItem.setAttribute('class','form-group amberu-pricelist-form-group');
					...
					newItem.innerHTML = '\
						<h5>Прайслист № ' + newItemNumber + '</h5>\
						<label class="col-sm-2 control-label" for="input-amberu-pricelist-' + newItemNumber + '-order-limit">\
							<span data-original-title="Лимит прайслиста №' + newItemNumber + '" data-toggle="tooltip" title="Лимит прайслиста №' + newItemNumber + '">Лимит ' + newItemNumber + '</span>\
						</label>\
						<div class="col-sm-4">\
							<input name="config_amberu_pricelists[' + newItemNumber + '][order_limit]" value="" placeholder="Сумма" id="input-amberu-pricelist-' + newItemNumber + '-order-limit" class="form-control" onkeyup="amberuTrimInputValue(this)" type="text">\
						</div>\
						<label class="col-sm-2 control-label" for="select-amberu-pricelist-' + newItemNumber + '-customer-group">\
							<span data-original-title="Доступ для прайслиста №' + newItemNumber + '" data-toggle="tooltip" title="Доступ к прайслисту №' + newItemNumber + '">Доступ</span>\
						</label>\
						<select name="config_amberu_pricelists[' + newItemNumber + '][customer_group_id]" class="form-control col-sm-4" id="select-amberu-pricelist-' + newItemNumber + '-customer-group">\
							' + selectForCustomerGroup.innerHTML + '\
						</select>\
					';
					itemParent.appendChild(newItem);
					//setItemCount("increment");
				}
				function amberuTrimInputValue(input){
					patternDigitDotSymboles = /[^\d.]/ig;
					input.value = input.value.replace(patternDigitDotSymboles, '');
					//replace repeating dots
					patternDigitDotSymboles = /\.+/ig;
					input.value = input.value.replace(patternDigitDotSymboles, '.');
					//replace more than one dot(not repeating)
					var firstDotPosition = input.value.indexOf('.');//first dot
					var secondDotPosition = input.value.indexOf('.',firstDotPosition+1);//there would be not third or more dots, 'cause of repeating dots replacing
					if (secondDotPosition != -1) {
						//substring() returns substring from indexA to indexB except indexB position: [indexA, indexB).
						//so copy substring from 0 to secondDotPosition + from secondDotPosition+1 to the end of String
						input.value = input.value.substring(0, secondDotPosition) + input.value.substring(secondDotPosition+1);
					}
				}
			</script>
			<select class="amberu-display-none" id = "select-amberu-pricelist-blank-customer-group">
						<option value="-1" selected="selected"><?php echo "Для всех"; ?></option>
			<?php	foreach ($customer_groups as $customer_group) {  ?>
						<option value="<?php echo $customer_group['customer_group_id']; ?>"><?php echo $customer_group['name']; ?></option>
			<?php 	} ?>
