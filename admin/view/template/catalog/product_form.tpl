      ...
      <div class="panel-body">
        <form action="<?php echo $action; ?>" method="post" enctype="multipart/form-data" id="form-product" class="form-horizontal">
          <ul class="nav nav-tabs">
			...
			<li><a href="#tab-amberu-prices" data-toggle="tab"><?php echo "Amberu Multiprice"; ?></a></li>
          ...
          <div class="tab-content">
			...
			<div class="tab-pane" id="tab-amberu-prices">
				  <h2>AMBERU MULTIPRICE</h2>
				  <?php if (empty($amberu_multiprice_categories)) { ?>
					  <div id="caption-empty-multiprice_categories">
							<br />Мульти-ценовые категории отсутсвуют. Отметьте одну или более категорию, в настройках, как мультиценовую.
					  </div>
				  <?php } else 	{ ?>
				  ...
				  <?php 		} ?>
			  <div style="<?php echo (!empty($amberu_multiprice_categories)) ? "display: block;" : "display: none;";?>">
				  <div class="form-group">
					<label id="amberu-multiprice-label" class="col-sm-2 control-label" for="input-amberu-multiprice"><span data-toggle="tooltip" title="<?php echo "Сделать продукт мульти-ценовым"; ?>"><?php echo "Мульти-цена"; ?></span></label>
					<div class="col-sm-10">
					  <div class="checkbox">
						<label>
						  <?php if ((!empty($amberu_multiprice_categories)) && $amberu_multiprice) { ?>
						  <input type="checkbox" name="amberu_multiprice" value="1" checked="checked" id="input-amberu-multiprice" onchange="amberuCheckboxChange(this)" />
						  <?php } else { ?>
				...
				<div id="amberu-prices-container" style="<?php echo $amberu_multiprice ? "display: block;" : "display: none;";?>">
					<?php 
						...
						for($amberu_i = 0; $amberu_i < $amberu_pricelists_count; $amberu_i++) {
							$amberu_price_count++;
					?>
							<div id="amberu-price-<?php echo $amberu_price_count; ?>-container" class="form-group">
								<label class="col-sm-2 control-label" for="input-amberu-price-<?php echo $amberu_price_count; ?>">
									<span data-toggle="tooltip" title="<?php echo "Цена прайслиста №", $amberu_price_count; ?>"><?php echo "Цена ", $amberu_price_count; ?>
									</span>
								</label>
								<div class="col-sm-10">
			...
			<script type="text/javascript">
				//amberuSubmitForm()
				function amberuSubmitForm() {
					var flag = false;
					var checkbox = document.getElementById('input-amberu-multiprice');
					if(!checkbox.checked) {
						//can submit
						flag = true;
					}
						...
						var lastItemContainer = getLastItem(itemParent.children, /amberu-price-\d+-container/ig);
						var lastItemNumber = getItemNumber(lastItemContainer, /price-(\d*)/ig);
						var el;
						for(var i = 1; i <= lastItemNumber; i++) {
							el = document.getElementById('input-amberu-price-' + i);
				...
				function changePricesNumber(pricesCount){
					...
					var lastItemContainer = getLastItem(itemParent.children, /amberu-price-\d+-container/ig);
					var lastItemNumber = getItemNumber(lastItemContainer, /price-(\d*)/ig);
					if(lastItemNumber > pricesCount) {
						for(var i = lastItemNumber; i > pricesCount; i--) {
							amberuRemoveItem(itemParent.id);
						}
					}
					else if(lastItemNumber < pricesCount) {
						for(var i = parseInt(lastItemNumber)+1; i <= pricesCount; i++) {
				...
				function amberuCheckboxChange(el) {
					if (el.checked) {
						document.getElementById('amberu-prices-container').style.cssText = 'display: block;';
					}
					else {
						document.getElementById('amberu-prices-container').style.cssText = 'display: none;';
					}
				}
				...
				function getItemNumber(item, patternItemNumber){
					...
					return itemParentNumber = matches[1];
				}
				...
				function amberuRemoveItem(mainParentId) {
					...
					else alert("Спокойно. Полей для удаления больше нет");
				}
				function amberuAddItem(mainParentId) {
					...
					newItem.innerHTML = '\
						...
						<div class="col-sm-10">\
							<input type="text" name="amberu_price_array[' + newItemNumber + ']" value="" \
								placeholder="Цена" id="input-amberu-price-' + newItemNumber + '" class="form-control" \
								onkeyup="amberuTrimInputValue(this)"\
							/>\
						</div>\
					';
            ...
            <div class="tab-pane" id="tab-image">
			  ...
			  <style>
				...
				#amberu-images-to-option-container .amberu-info {
					font-size: 13px;
					margin-bottom: 10px;
					text-align: left;
					padding: 4px 10px;
				...
				#amberu-images-to-option-container select {
					width: 200px;
					display: inline-block;
				}
				#amberu-images-to-option-container h4 {
					display: inline-block;
				}
			  </style>
			  <div id="amberu-images-to-option-container">
			    <h3><?php echo 'AMBERU'; ?></h3>
				<div class="amberu-info">
				  <i class="fa fa-exclamation"></i> <?php echo $amberu_info_images_to_option; ?>
				</div>
				<h4><?php echo $amberu_text_images_to_option . ': '; ?></h4>
                ...
                <table id="images" class="table table-striped table-bordered table-hover">
                  <thead>
                    <tr>
					  ...
					  <td class="text-right"><?php echo $entry_option_value; ?></td>
                      <td></td>
                    </tr>
                  </thead>
                  <tbody>
                    ...
                    <tr id="image-row<?php echo $image_row; ?>">
                      ...
                      <td class="text-left">
					    <select name="product_image[<?php echo $image_row; ?>][amberu_option_value_id]" class="form-control amberu-image-to-option-value-select">
						  <?php if(!$amberu_images_to_options) { ?>
						  <option value="0"><?php echo $amberu_text_choose_option; ?></option>
						  <?php } else { ?>
						    <option value="0"><?php echo $amberu_text_missing; ?></option>
					  ...
					  <td class="text-left"><button type="button" onclick="$(this).tooltip('hide');$('#image-row<?php echo $image_row; ?>').remove();amberuImagesRemoveButton();" data-toggle="tooltip" title="<?php echo $button_remove; ?>" class="btn btn-danger"><i class="fa fa-minus-circle"></i></button></td>
                    </tr>
                    <?php $image_row++; ?>
                    <?php } ?>
                  </tbody>
...
$('select[name=\'amberu_images_to_options\']').change( function(e) {
	...
	if (($(e['delegateTarget']).val() > 0) && ( $( ".amberu-image-to-option-value-select" ).length )) {
		$('.amberu-image-to-option-value-select').html('<option value="0">' + 'Загрузка...' + '</option>');
		$.ajax({
			url: 'index.php?route=catalog/option/amberuGetOptionValues&token=<?php echo $token; ?>',
			type: 'post',
			...
			success: function(json) {
				...
				for (var i=0; i < json['option_value'].length; i++) {
					html +=
					'<option value="' + json['option_value'][i]['option_value_id'] + '">' + json['option_value'][i]['name'] + '</option>'
					;
				}
				$('.amberu-image-to-option-value-select').html(html);
				$('#images tfoot button').removeClass('disabled');
			},
		...
		$('select[name=\'amberu_images_to_options\'] option:selected').removeAttr('selected');
		$('#images tfoot button').removeClass('disabled');
	}
});
function amberuImagesRemoveButton() {
	if (!$('#images tbody tr').length) {
		$('select[name=\'amberu_images_to_options\'] option:selected').removeAttr('selected');
	}
}
</script>
...
function addImage() {
	...
	html +=
		'<td class="text-left">\
			<select name="product_image[' + image_row + '][amberu_option_value_id]" class="form-control amberu-image-to-option-value-select">\
				' + amberuSelectInnerHtml + '\
			</select>\
		</td>'
	;
