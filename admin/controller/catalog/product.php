...
class ControllerCatalogProduct extends Controller {
	...
	protected function getForm() {
		...
		//AMBERU
		//amberu_multiprice
			if (isset($this->request->post['amberu_multiprice'])) {
				$data['amberu_multiprice'] = $this->request->post['amberu_multiprice'];
			} elseif (!empty($product_info)) {
				$data['amberu_multiprice'] = $product_info['amberu_multiprice'];
			} else {
				$data['amberu_multiprice'] = 0;
			}
		//amberu_prices
			if (isset($this->request->post['amberu_price_array'])) {
				$data['amberu_price_array'] = $this->request->post['amberu_price_array'];
			} elseif (!empty($product_info)) {
				if ($product_info['amberu_prices'] != "")
					$data['amberu_price_array'] = explode(",", $product_info['amberu_prices']);
				else
					$data['amberu_price_array'] = "";
			} else {
				$data['amberu_price_array'] = "";
			}
		//amberu_images_to_options
			if (isset($this->request->post['amberu_images_to_options'])) {
				$data['amberu_images_to_options'] = $this->request->post['amberu_images_to_options'];
			} elseif (!empty($product_info)) {
				$data['amberu_images_to_options'] = $product_info['amberu_images_to_options'];
			} else {
		...
		$data['amberu_pricelists_count'] = count($amberu_setting_config['config_amberu_pricelists']);
		$amberu_categories = $this->model_catalog_category->getCategories();
		$data['amberu_multiprice_categories'] = array();
		foreach($amberu_categories as $amberu_category) {
			$amberu_category_info = $this->model_catalog_category->getCategory($amberu_category['category_id']);
			if ($amberu_category_info['amberu_multiprice']) {
				$data['amberu_multiprice_categories'][] = $amberu_category_info;
			}
		}
		...
		$data['amberu_text_images_to_option'] = $this->language->get('amberu_text_images_to_option');
		$data['amberu_info_images_to_option'] = $this->language->get('amberu_info_images_to_option');
		$data['amberu_text_missing'] = $this->language->get('amberu_text_missing');
		$data['amberu_text_choose_option'] = $this->language->get('amberu_text_choose_option');
		
		$data['amberu_images_to_options_values'] = 
			$data['amberu_images_to_options'] > 0 ? $this->model_catalog_option->getOptionValues($data['amberu_images_to_options']) : null;
		
		$data['amberu_images_options'] = array();
		foreach($this->model_catalog_option->getOptions() as $amberu_option) {
			//only type==select by now
			if ($amberu_option['type'] == 'select') {
				$data['amberu_images_options'][] =  $amberu_option;
			}
		}
		//end
		...
		foreach ($product_images as $product_image) {
			...
			$data['product_images'][] = array(
				...
				//AMBERU
				'amberu_option_value_id' => $product_image['amberu_option_value_id'],
			);
		}
