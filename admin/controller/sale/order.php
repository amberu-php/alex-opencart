...
class ControllerSaleOrder extends Controller {
	...
	protected function getList() {
		...
		foreach ($results as $result) {
			$data['orders'][] = array(
				...
				//AMBERU
				'amberu_pricelist_number'      => $result['amberu_pricelist_number'],
				'amberu_note'      => $result['amberu_note'],
		...
		$data['amberu_text_amberu_note'] = $this->language->get('amberu_text_amberu_note');
	...
	public function getForm() {
		...
		//language
		$this->load->language('amberu/multiprice');
		$data['amberu_text_pricelist_number'] = $this->language->get('amberu_text_pricelist_number');
		$data['amberu_text_order_limit'] = $this->language->get('amberu_text_order_limit');
		...
		//data
		$data['amberu_pricelist_number'] = (!empty($order_info)) ? $order_info['amberu_pricelist_number'] : 0;
		...
		foreach ($data['amberu_pricelists'] as $amberu_key => $amberu_pricelist) {
			if ($amberu_pricelist['order_limit'] > 0) {
				...
				$amberu_pricelist['order_limit'] = sprintf($data['amberu_text_order_limit'], $amberu_pricelist['order_limit']);
				$data['amberu_pricelists'][$amberu_key]['order_limit_text'] = $amberu_pricelist['order_limit'];
			}
			else {
				$data['amberu_pricelists'][$amberu_key]['order_limit_text'] = $data['amberu_text_retail'];
			}
		}
		//END
	...
	public function info() {
		...
		if ($order_info) {
			...
			$data['amberu_text_retail'] = $this->language->get('amberu_text_retail');
			$data['amberu_text_amberu_note'] = $this->language->get('amberu_text_amberu_note');
			$data['amberu_btn_amberu_note'] = $this->language->get('amberu_btn_amberu_note');
			
			//data
			$data['amberu_pricelist_number'] = $order_info['amberu_pricelist_number'];
			$this->load->model('setting/setting');
			$amberu_setting_config = $this->model_setting_setting->getSetting('config');
			$amberu_pricelists = $amberu_setting_config['config_amberu_pricelists'];
			if ($data['amberu_pricelist_number'] > 0) {
				$data['amberu_order']['order_limit'] = $amberu_pricelists[$data['amberu_pricelist_number']]['order_limit'];
				$data['amberu_order']['text_order_limit'] = 
					$this->currency->format($data['amberu_order']['order_limit'], $this->currency->getCode(), 1, true);
				$data['amberu_text_order_limit'] = sprintf($data['amberu_text_order_limit'], $data['amberu_order']['text_order_limit']);
			}
			
			$data['amberu_note'] = $order_info['amberu_note'];
			//END
	...
	public function invoice() {
		...
		$data['amberu_text_missing'] = $this->language->get('amberu_text_missing');
		$data['text_customer'] = $this->language->get('text_customer');
		$data['amberu_text_ship_to'] = $this->language->get('amberu_text_ship_to');
		$data['text_country'] = $this->language->get('text_country');
		$data['text_zone'] = $this->language->get('text_zone');
		$data['text_city'] = $this->language->get('text_city');
		$data['text_postcode'] = $this->language->get('text_postcode');
		...
		foreach ($orders as $order_id) {
			...
			if ($order_info) {
				...
				//AMBERU
				$amberu_customer_name= $order_info['firstname'] . ' ' . $order_info['lastname'];
				$amberu_customer_name = trim(preg_replace('/\s+/', ' ', $amberu_customer_name));
				$amberu_shipping_country = $order_info['shipping_country'];
				$amberu_shipping_zone = $order_info['shipping_zone'];
				$amberu_shipping_city = $order_info['shipping_city'];
				$amberu_shipping_postcode = $order_info['shipping_postcode'];
				$amberu_shipping_address_1 = $order_info['shipping_address_1'];
				$amberu_shipping_address_2 = $order_info['shipping_address_2'];
				//
				//AMBERU
				$this->load->model('catalog/product');
				$amberu_products_quantity = 0;
				//
				...
				foreach ($products as $product) {
					...
					//AMBERU
					$amberu_products_quantity += $product['quantity'];
					$amberu_product_info = $this->model_catalog_product->getProduct($product['product_id']);
					$amberu_sku = $amberu_product_info['sku'];
					//
				...
				$data['orders'][] = array(
					'order_id'	         => $order_id,
					//amberu
					'amberu_pricelist_number' => $order_info['amberu_pricelist_number'],
					'amberu_order_limit_text' => 
						($order_info['amberu_pricelist_number'] > 0) ? 
							$data['amberu_pricelists'][$order_info['amberu_pricelist_number']]['order_limit_text'] : $data['amberu_text_retail'],
					'amberu_products_quantity' => $amberu_products_quantity,
					//
					...
					'store_fax'          => $store_fax,
					//amberu
					'amberu_customer_name' => $amberu_customer_name,
					'amberu_shipping_country' => $amberu_shipping_country,
					'amberu_shipping_zone' => $amberu_shipping_zone,
					'amberu_shipping_city' => $amberu_shipping_city,
					'amberu_shipping_postcode' => $amberu_shipping_postcode,
					'amberu_shipping_address_1' => $amberu_shipping_address_1,
					'amberu_shipping_address_2' => $amberu_shipping_address_2,
					//
	...
	public function api() {
		...
		if (isset($this->session->data['cookie']) && isset($this->request->get['api'])) {
			...
			//AMBERU
			if (isset($this->request->post['amberu_pricelist_number'])) {
				$url_data['amberu_pricelist_number'] = $this->request->post['amberu_pricelist_number'];
			}
			if (isset($this->request->post['quantity'])) {
				$this->request->post['quantity'] = abs((int)$this->request->post['quantity']);
				$this->request->post['quantity'] = $this->request->post['quantity'] > 0 ? $this->request->post['quantity'] : 1;
			}
			//end
	...
	//AMBERU
	public function amberuEditAmberuNote() {
		$this->load->language('amberu/multiprice');
		
		$json = array();
		
		if ((isset($this->request->post['amberu_note'])) && (isset($this->request->post['order_id']))) {
			$this->load->model('sale/order');
			$this->model_sale_order->amberuEditAmberuNote($this->request->post['order_id'], $this->request->post['amberu_note']);
			$json['success'] = $this->language->get('amberu_succes_edit_amberu_note');
		}
		else {
			$json['error'] = $this->language->get('amberu_error_edit_amberu_note');
		}
